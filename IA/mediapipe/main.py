...
window = pygame.display.set_mode(
    (band_width * magnification, band_height * magnification))  # Adjusted window size for 2D tracking
...
def magnify_screen(x_position, y_position):
    ...
    band = magnified.subsurface((x_position * magnification, y_position * magnification, band_width * magnification,
                                 band_height * magnification))  # Adjusted subsurface for 2D tracking
    return band
...
with mp_face_mesh.FaceMesh(min_detection_confidence=0.7,
                           min_tracking_confidence=0.7) as face_mesh:  # Increased confidence values for more sensitivity
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                if distance_moved > 0.02:  # Made it 5 times more sensitive to movement
                    ...
                    gaze_position_y = (nose_tip.y * roi_height + roi_y) * pyautogui.size()[
                        1] / height  # Adjusted for ROI
                    print(gaze_position_x, gaze_position_y)
